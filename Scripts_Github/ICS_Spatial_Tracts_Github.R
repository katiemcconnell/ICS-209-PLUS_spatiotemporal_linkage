...
tracts <- st_read("nhgis0024_shapefile_tl2020_us_tract_2020/US_tract_2020.shp") %>%
  st_transform(crs = st_crs(mtbs)) # change coordinate system to match MTBS
...
# Spatial join (MTBS footprints & tracts) ----------------------------------------------------

# Join MTBS footprints with ICS data (tabular join based on MTBS ID)
ics_join_mtbs_tracts <- inner_join(mtbs2, ics7, by = c("Event_ID" = "MTBS_ID_FINAL")) %>%
  dplyr::select(-(irwinID:X1), -(Event_ID)) %>% # remove extraneous MTBS columns
  rename(INCIDENT_ID = INCIDENT_ID.y)

# Spatial join ICS-MTBS polygons with spatial units (tracts) 
ics_join_mtbs_tracts2 <- st_join(ics_join_mtbs_tracts, tracts, left = TRUE) %>% # left = TRUE makes a left join
  dplyr::select(-(POO_LATITUDE:POO_LONGITUDE)) %>%
  mutate(MTBS_ID_FINAL = NA) %>%
  dplyr::select(-(MTFCC:Shape_Leng)) %>% # remove extraneous columns from tracts data
...
# Spatial join (FIRED & tracts) -----------------------------------------

# Join FIRED footprints with ICS data (tabular)
ics_join_fired_tracts <- inner_join(fired, ics7, by = c("INCI_ID" = "INCIDENT_ID")) 

# Spatial join ICS-FIRED polygons with spatial units (tracts)
ics_join_fired_tracts2 <- st_join(ics_join_fired_tracts, tracts, left = TRUE) %>% # left = TRUE makes a left join
  dplyr::select(-(POO_LATITUDE:POO_LONGITUDE)) %>%
...
# Combine MTBS and FIRED tracts -----------------------------------------

# Join MTBS and FIRED tract designations 
...
mtbs_fired_tracts <- full_join(ics_join_mtbs_tracts2, 
                                 ics_join_fired_tracts2,
...
mtbs_only_geoid_tract <- mtbs_fired_tracts %>% filter(Spatial_Join_MTBS == 1 & is.na(Spatial_Join_FIRED)) # only MTBS
fired_only_geoid_tract <- mtbs_fired_tracts %>% filter(Spatial_Join_FIRED == 1 & is.na(Spatial_Join_MTBS)) # only FIRED
mtbs_fired_both_geoid_tract <- mtbs_fired_tracts %>% filter(Spatial_Join_FIRED == 1 & Spatial_Join_MTBS == 1) # both

# Check to make sure that all observations are designated to MTBS, FIRED, or both (below should be TRUE)
nrow(mtbs_only_geoid_tract) + nrow(fired_only_geoid_tract) + nrow(mtbs_fired_both_geoid_tract) == nrow(mtbs_fired_tracts)

# Spatial join (POO & tracts) ---------------------------------------------

# Filter to all remaining fire incidents that don't have MTBS or FIRED spatial information
ics8 <- ics7 %>% filter(!(INCIDENT_ID %in% mtbs_fired_tracts$INCIDENT_ID)) %>% # remove incidents that are already processed with MTBS or FIRED
...
length(unique(ics8$INCIDENT_ID)) + length(unique(mtbs_fired_tracts$INCIDENT_ID)) == length(unique(ics7$INCIDENT_ID))

# Convert lat/long to spatial points 
ics_points <- st_as_sf(ics8, 
                       coords = c("POO_LONGITUDE", "POO_LATITUDE"), 
                       crs = st_crs(tracts), # set CRS to match spatial units
                       na.fail = FALSE)

# Join ICS points to spatial units 
ics_points_joined_tracts <- st_join(ics_points, tracts, left = TRUE) %>% # left = TRUE makes a left join
  dplyr::select(-(X1)) %>%
  mutate(MTBS_ID_FINAL = NA) %>%
  dplyr::select(-(MTFCC:Shape_Leng)) %>% # remove extraneous columns from tracts data
  mutate(Spatial_Join_FIRED = NA,# create empty columns to match mtbs_fired_tracts for rbind below
...
# Combine tract designations from MTBS + FIRED and POO points ---------------------------------------------

# Combine ICS points and polygons 
ics_spatial_tracts <- bind_rows(mtbs_fired_tracts, ics_points_joined_tracts) %>% 
  mutate(GEOID = as.character(GEOID)) %>%
...
nrow(mtbs_fired_tracts) + nrow(ics_points_joined_tracts) == nrow(ics_spatial_tracts)

# Check for any duplicates, there should be no repeated Incident_ID-GEOID combinations (nrow(dup_check2) should be 0)
dup_check <- ics_spatial_tracts %>% group_by(INCIDENT_ID, GEOID) %>% summarise(n_observations = n())
...
missing_tracts <- filter(ics_spatial_tracts, is.na(GEOID)) 
1 - (nrow(missing_tracts) / length(unique(ics4$INCIDENT_ID))) # percent of ICS database with any spatial join

# Calculate % of incidents that overlap multiple tracts -----------------

# Summarize total number of unique GEOID's per wildfire Incident ID
multiple_geoid_per_incident <- ics_spatial_tracts %>%
  group_by(INCIDENT_ID) %>%
  summarise(n_obs = n()) %>%
  filter(n_obs > 1) # Pull out only incidents with 2 or more GEOID's

# Calculate proportion of incidents that cross multiple tracts out of all incidents
...
ics_spatial_tracts2 <- mutate(ics_spatial_tracts, 
...
ics_spatial_tracts3 <- ics_spatial_tracts2 %>%
...
write_csv(ics_spatial_tracts3, "ics_spatial_tracts.csv")
...
multiple_quarter_per_incident <- ics_spatial_tracts2 %>%
