...
# Spatial join (MTBS footprints & CBG's) ----------------------------------------------------

# Join MTBS footprints with ICS data (tabular join based on MTBS ID)
ics_join_mtbs_cbg <- inner_join(mtbs2, ics7, by = c("Event_ID" = "MTBS_ID_FINAL")) %>%
  dplyr::select(-(irwinID:X1), -(Event_ID)) %>% # remove extraneous MTBS columns
  rename(INCIDENT_ID = INCIDENT_ID.y)

# Spatial join ICS-MTBS polygons with spatial units (block groups) 
ics_join_mtbs_cbg2 <- st_join(ics_join_mtbs_cbg, cbg, left = TRUE) %>% # left = TRUE makes a left join
  dplyr::select(-(POO_LATITUDE:POO_LONGITUDE)) %>%
  mutate(MTBS_ID_FINAL = NA) %>%
  dplyr::select(-(MTFCC:Shape_Area)) %>% # remove extraneous columns from counties data
...
# Spatial join (FIRED & CBG's) -----------------------------------------

# Join FIRED footprints with ICS data (tabular)
ics_join_fired_cbg <- inner_join(fired, ics7, by = c("INCI_ID" = "INCIDENT_ID")) 

# Spatial join ICS-FIRED polygons with spatial units (cbg's)
ics_join_fired_cbg2 <- st_join(ics_join_fired_cbg, cbg, left = TRUE) %>% # left = TRUE makes a left join
  dplyr::select(-(POO_LATITUDE:POO_LONGITUDE)) %>%
...
mtbs_fired_cbg <- full_join(ics_join_mtbs_cbg2, 
                               ics_join_fired_cbg2,
...
mtbs_only_geoid_cbg <- mtbs_fired_cbg %>% filter(Spatial_Join_MTBS == 1 & is.na(Spatial_Join_FIRED)) # only MTBS
fired_only_geoid_cbg <- mtbs_fired_cbg %>% filter(Spatial_Join_FIRED == 1 & is.na(Spatial_Join_MTBS)) # only FIRED
mtbs_fired_both_geoid_cbg <- mtbs_fired_cbg %>% filter(Spatial_Join_FIRED == 1 & Spatial_Join_MTBS == 1) # both

# Check to make sure that all observations are designated to MTBS, FIRED, or both (below should be TRUE)
nrow(mtbs_only_geoid_cbg) + nrow(fired_only_geoid_cbg) + nrow(mtbs_fired_both_geoid_cbg) == nrow(mtbs_fired_cbg)

# Spatial join (POO & CBG's) ---------------------------------------------

# Filter to all remaining fire incidents that don't have MTBS or FIRED spatial information
ics8 <- ics7 %>% filter(!(INCIDENT_ID %in% mtbs_fired_cbg$INCIDENT_ID)) %>% # remove incidents that are already processed with MTBS or FIRED
...
length(unique(ics8$INCIDENT_ID)) + length(unique(mtbs_fired_cbg$INCIDENT_ID)) == length(unique(ics7$INCIDENT_ID))

# Convert lat/long to spatial points 
ics_points <- st_as_sf(ics8, 
                       coords = c("POO_LONGITUDE", "POO_LATITUDE"), 
                       crs = st_crs(cbg), # set CRS to match spatial units
                       na.fail = FALSE)

# Join ICS points to spatial units 
ics_points_joined_cbg <- st_join(ics_points, cbg, left = TRUE) %>% # left = TRUE makes a left join
  dplyr::select(-(X1)) %>%
  mutate(MTBS_ID_FINAL = NA) %>%
  dplyr::select(-(MTFCC:Shape_Area)) %>% # remove extraneous columns from CBG data
  mutate(Spatial_Join_FIRED = NA,# create empty columns to match mtbs_fired_cbg for rbind below
...
# Combine CBG designations from MTBS + FIRED and POO points ---------------------------------------------

# Combine ICS points and polygons 
ics_spatial_cbg <- bind_rows(mtbs_fired_cbg, ics_points_joined_cbg) %>% 
  mutate(GEOID = as.character(GEOID)) %>%
...
nrow(mtbs_fired_cbg) + nrow(ics_points_joined_cbg) == nrow(ics_spatial_cbg)

# Check for any duplicates, there should be no repeated Incident_ID-GEOID combinations (nrow(dup_check2) should be 0)
dup_check <- ics_spatial_cbg %>% group_by(INCIDENT_ID, GEOID) %>% summarise(n_observations = n())
...
missing_cbg <- filter(ics_spatial_cbg, is.na(GEOID)) 
1 - (nrow(missing_cbg) / length(unique(ics4$INCIDENT_ID))) # percent of ICS database that has a spatial join

# Calculate % of incidents that overlap multiple CBG's -----------------

# Summarize total number of unique GEOID's per wildfire Incident ID
multiple_geoid_per_incident_cbg <- ics_spatial_cbg %>%
  group_by(INCIDENT_ID) %>%
  summarise(n_obs = n()) %>%
  filter(n_obs > 1) # Pull out only incidents with 2 or more GEOID's

# Calculate proportion of incidents that cross multiple CBG's out of all incidents
length(unique(multiple_geoid_per_incident_cbg$INCIDENT_ID)) / length(unique(ics4$INCIDENT_ID))

# Calculate Quarters -----------------------------------------------

# Designate quarter and year (should increase the number of observations from ics_spatial_cbg, as fires across >1 quarter are broken into Fire-GEOID-Quarter)
ics_spatial_cbg2 <- mutate(ics_spatial_cbg, 
